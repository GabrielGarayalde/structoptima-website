...
export function axialbendingCapacity(phi, N_load, kf, fy, Ag, I_x, Le_x, Ze_x) {
  ...
  const alpha_a_x = (2100 * (lambda_n_x - 13.5)) / (Math.pow(lambda_n_x, 2) - 15.3 * lambda_n_x + 2050);
  ...
  const xi_x = (Math.pow(lambda_x / 90, 2) + 1 + eta_x) / (2 * Math.pow(lambda_x / 90, 2)); //Factor
  const alpha_c_x = xi_x * (1 - Math.sqrt(1 - Math.pow(90 / (xi_x * lambda_x), 2))); // slenderness reduction factor
